...
export class ScatterplotComponent {
  @Input() twitterState: TwitterState;
  ...
  setup() {
    ...
    this.rScale = D3.scaleLinear().range([2, 15]);
  }
  ...
  populate() {
    if (this.twitterState.tweets) {
      ...
      this.rScale.domain([0, this.getMax('statuses_count')]);
      this.svg.selectAll('.dot')
        .data(this.twitterState.tweets)
        .enter().append('circle')
          .attr('class', 'dot')
          .attr('r', d => this.rScale(d.statuses_count))
